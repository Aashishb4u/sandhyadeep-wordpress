			...
			<div class="col-md-10 col-lg-8">
				<div class="section-title title-01 mb-70">	
					<span class="section-id">
						<?php if(isset($lanotte_redux_demo['author_title']) && $lanotte_redux_demo['author_title']!=''){?>
						<?php echo wp_specialchars_decode(esc_attr($lanotte_redux_demo['author_title']));?>
						<?php }else{?>
						<?php echo esc_html__( 'All post by author', 'lanotte' );
						}?>
					</span>	
					<h2 class="h2-lg text-cap">
						<?php printf( esc_html__( '%s', 'lanotte' ), get_the_author() );?>
					</h2>
				</div>	
			</div>
		</div>
