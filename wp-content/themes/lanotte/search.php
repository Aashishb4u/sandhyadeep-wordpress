			...
			<div class="col-md-10 col-lg-8">
				<div class="section-title title-01 mb-70">	
					<span class="section-id">
						<?php if(isset($lanotte_redux_demo['search_title']) && $lanotte_redux_demo['search_title']!=''){?>
						<?php echo wp_specialchars_decode(esc_attr($lanotte_redux_demo['search_title']));?>
						<?php }else{?>
						<?php echo esc_html__( 'Search Results for', 'lanotte' );
						}?>
					</span>	
					<h2 class="h2-lg text-cap">
						<?php printf( esc_html__( '%s', 'lanotte' ), get_search_query() );?>
					</h2>
				</div>	
			</div>
		</div>
			...
			<?php else: ?>
				<div class="col-lg-10 offset-lg-1">
					<div class="search_custom">
						<h4><?php echo wp_specialchars_decode(esc_attr($lanotte_redux_demo['search_no_match']));?></h4>
						<?php get_search_form(); ?>
					</div>
				</div>
			<?php endif; ?>
