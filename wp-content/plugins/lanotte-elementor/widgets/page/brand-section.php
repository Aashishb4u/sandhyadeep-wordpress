...
class BdevsBrandSection extends \Elementor\Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'tabs',
			[
				'label' => esc_html__( 'Section Brand Items', 'bdevs-elementor' ),
				...
				<div class="owl-carousel brands-1-carousel">
					...
					?>
						<?php if ('' != $item['link_item']) { ?>
							<a class="s_1" href="<?php echo wp_kses_post($item['link_item']); ?>">
								<img src="<?php echo wp_kses_post($item['img_item']['url']); ?>" width="180">
							</a>
						<?php } else { ?>
							<div class="brand-logo">
								<img class="img-fluid" src="<?php echo wp_kses_post($item['img_item']['url']); ?>" alt="brand-logo" />
							</div>
						<?php } ?>
					<?php endforeach; ?>
				</div>
